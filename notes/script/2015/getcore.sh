...
echo "<tr><td><h2>101调试集群</h2></td></tr>"
...
echo "<tr><td>$srv"
d1=`date`
wget --tries=2 --timeout=5 -q "http://$srv:13160/recommend/get_top_recommend?accesstoken=TOKEN50002999&label=101|102|107|103&num=15|15|15|15&hdsize=163x198&sdsize=163x198&appsize=163x198&vodsize=163x198&chnlsize=163x198" -O reslave
...
cmd=`ssh $srv 'ls -al /homed/*/bin/core.*'`
echo $cmd
...
echo "<tr><td>$srv "
d1=`date`
wget --tries=1 --timeout=5 -q "http://$srv:12690/getIaps" -O reiacs
...
echo "<tr><td>$dtvsrv "
d1=`date`
wget --tries=1 --timeout=5 -q "http://$dtvsrv:12890/media/transmission/get_list?accesstoken=TOKEN1000000&status=810|818|819|820|829|830|838|839|840|848|849|890|898|899|900|910|920|930|940&pagenum=50&pageidx=1" -O redtvs
[[ `grep -c "video_list" redtvs` -eq 0 ]] && echo "<B><font color=red> Error </font></B> </td></tr>"
[[ `grep -c "video_list" redtvs` -ne 0 ]] && echo "<B><font color=green> OK </font></B> </td></tr>"
echo "<tr><td></td></tr>"
rm -f redtvs
done;
exit 0
